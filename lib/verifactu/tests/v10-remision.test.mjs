...
// V24: xmlDeFila ya NO confía en el XML fijado (siempre regenera desde el
// jsonb y verifica huella + coincidencia byte a byte), así que las filas de
// los tests llevan un registro persistido REAL y reconstruible.
const SI_TEST = sistemaInformaticoKuentas('KU-test-0001')
const FECHA_GEN = new Date('2026-09-12T10:00:00+02:00')
const cacheRegistros = new Map()

function registroReal(numSerie, tipo = 'alta') {
  const clave = `${tipo}:${numSerie}`
  if (cacheRegistros.has(clave)) return cacheRegistros.get(clave)
  const encadenamiento = { primerRegistro: true }
  const opciones = { encadenamiento, sistemaInformatico: SI_TEST, fechaGeneracion: FECHA_GEN }
  let entrada, generado
  if (tipo === 'alta') {
    entrada = {
    ...
    generado = construirRegistroAlta(entrada, opciones)
  } else {
    entrada = {
      emisor: { nif: NIF },
    ...
    generado = construirRegistroAnulacion(entrada, opciones)
  }
  const persistido = {
    entrada,
  ...
  const r = { persistido, huella: generado.huella, xml: generado.xml }
  cacheRegistros.set(clave, r)
  return r
}

function fila(extra = {}) {
  const numSerie = extra.num_serie_factura ?? 'F-2026-000123'
  const tipo = extra.tipo_registro ?? 'alta'
  const real = registroReal(numSerie, tipo)
  return {
...
test('xmlDeFila acepta el XML fijado solo si coincide byte a byte con el regenerado (V24)', () => {
  const f = fila()
  assert.equal(xmlDeFila(f), registroReal('F-2026-000123').xml)
})

test('xmlDeFila rechaza un XML fijado manipulado aunque la huella del registro sea válida (V24)', () => {
  const f = fila({ xml: '<sf:RegistroAlta>fijado a mano</sf:RegistroAlta>' })
  assert.throws(() => xmlDeFila(f), /no coincide con el regenerado/)
})
...
test('remitirObligado: envío correcto → resolver_lote con estados, CSV y TiempoEsperaEnvio', async () => {
  ...
  assert.match(peticiones[0].cuerpo, /<sf:NumSerieFactura>F-2026-000123<\/sf:NumSerieFactura>/)
  assert.match(peticiones[0].cuerpo, /<sf:NumSerieFactura>F-2026-000124<\/sf:NumSerieFactura>/)
...
test('procesarRemision: sin obligados pendientes → resumen vacío', async () => {
  ...
  assert.deepEqual(r, { obligados: [], totalEnviados: 0, totalErrores: 0, totalOmitidos: 0 })
})
